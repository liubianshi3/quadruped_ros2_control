...
- 三次隔离重复（domain 219–221，提交 4537d1b 后执行）
  `/tmp/dog2_flat_liveness_repeat3_20260712/run_20260712T041050Z_5c0566c0/`：
  - 3/3 `STAGE_TIMEOUT`（OUTBOUND），零 TILT/走廊/base-contact/基础设施
    失败，零 FAULT——首跑的 TILT_LIMIT 未复现（0/3），维持 9c 证据但
    不升级优先级。
  - liveness 契约在压力下稳定：13 次 forced_settle（9/1/3），全部 1.20 s
    收敛且序列恢复，SWING 时长被硬性封顶在 1.20 s（此前失控到 9 s+）；
    trial_002 一次 SHIFT 在 8.02 s deadline 拍恰好 ready，按契约优先推进
    未误 FAULT。
  - cadence 归因三跑收敛：SHIFT 占走行预算 62.5/66.5/69.5%（均值
    2.0–2.5 s、最长 5.0–8.0 s，名义 0.45），SWING 23.6–30.1%（均值
    0.88–1.03 s），SETTLE ~7%；swing 数 15/12/13，投影
    0.349/0.187/0.258 m，推进约 4–7 mm/s。预算大头确定为 SHIFT 卸载等待。
  - 高 forced_settle 计数（尤其 trial_001 的 9/15）说明 touchdown 确认
    普遍慢于名义 0.45 s 摆窗，是 SWING 均值翻倍的直接构成。

### 续接点

- cadence 下一半批按归因定序：
  1. SHIFT 卸载等待（62–70% 预算）：拆"支撑目标 transit 时间"与"动态
     settle 门（速度 ≤0.025 与 0.10 m/s 预移速度自相矛盾的交替等待）"，
     与 9c 姿态刚度耦合部分不混调；
  2. touchdown 确认慢（forced_settle 高发）：核对 swing 下落段与接触
     事件延迟，必要时调 lower 段速度或 overshoot，不动 15 N 门；
  3. 全支撑段路径速度整段清零的必要性复核。
- TILT_LIMIT 复现率 0/3，暂不转 9c 批次；若后续批次中再现且集中在预移
  窗口，升级 9c。
