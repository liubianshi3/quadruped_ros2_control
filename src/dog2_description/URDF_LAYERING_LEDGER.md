...
**包路径**：与本文件同包 `dog2_description` 内的 urdf 分层 xacro 文件集（`dog2.urdf.xacro` 为装配入口 + `dog2_properties.xacro`/`dog2_leg_macro.xacro`/`dog2_ros2_control.xacro`）为运动学真值源。
...
| `leg_hip_rpy_R`（右腿髋静态 rpy — 遗留命名/台账项） | **已迁出 `coxa_joint` revolute 静态项**（**`rh` / `rf` 均同构**：`*_coxa_axis_fixed` + `*_coxa_pose_fixed`；`*_coxa_joint` 原点恒等、`axis` `0 0 -1`） | **A**（mount）若未来要再吸收：须完整 SE(3) 证明（Phase 2 已证平移闭合不够） | **证据**：原单关节 `rpy="π 0 π/2"`（及 `rf` 的 `hip_xyz`）与 **`hip_xyz` + 两段 fixed 的 RPY** 在 `rail→coxa_link` 上 **SE(3) 等价**（已用 Pinocchio / `test_lf_zero_pose_se3` / `check_joint_semantics` 验证）。当前 xacro 源码**不再定义** `leg_hip_rpy_R`，leg macro 也**不再接收/传入** `hip_rpy`；该名称仅作为历史台账口径保留。 | mount 误吸收仍高 | **`rh`：已清算。`rf`：已清算。** 历史量**从 revolute B 迁到 fixed** 已**全局结案**（就「是否仍压在 `coxa_joint` 上」而言）；**勿**说成「从未存在」或「与 hip 几何无关」 | 改右腿 coxa 模板、或 CAD 推翻等价分解时 | `check_joint_semantics.py`；`test_lf_zero_pose_se3.py`；`test_kinematics.py`；`migration_stage2_stand.json` |
...
## 与 URDF xacro 分层文件的对应关系（便于查代码）

- **`leg_thigh_mesh_flip_rpy`**：`dog2_leg_macro.xacro` `rf` 分支内 `thigh_vis_rpy` / `thigh_col_rpy_eff`（visual/collision `<origin rpy>`）。
- **`leg_hip_rpy_R`**：当前 xacro 源码**不再定义**该属性；`dog2_leg_macro.xacro` 的 `leg` macro **不再接收** `hip_rpy`，`dog2.urdf.xacro` 中 `rh` / `rf` 实例也**不传** `hip_rpy`。**`rh` / `rf`** 均在 `*_coxa_axis_fixed`、`*_coxa_pose_fixed` 承担髋静态姿态（与各自 `hip_xyz` 配合）。源码 `grep` 应**无** `hip_rpy="${leg_hip_rpy_R}"`。
- **`leg_knee_xyz_R`**：`dog2_properties.xacro` 定义；`dog2.urdf.xacro` 中 `rh`、`rf` 实例通过 `knee_xyz="${leg_knee_xyz_R}"` 传入；`dog2_leg_macro.xacro` 将该参数用于 `femur_axis_fixed` 的 `xyz`。
- **`lh` / `rf` `hip_xyz`**：`dog2.urdf.xacro` 中 `lh`、`rf` 实例各自 `hip_xyz="..."` 覆盖宏默认。
- **`base_link_semantic_origin_*`**：`dog2_properties.xacro` 定义；`dog2.urdf.xacro` 中 `base_link` 引用。
- **Joint limits / contact**：`dog2_properties.xacro`。
- **ros2_control plugin**：`dog2_ros2_control.xacro`。
...
- **仍须避免的过度表述**：**不要**把「清算完成」说成「右腿髋与历史 rpy 无关」——`leg_hip_rpy_R` 这个名称作为历史台账口径保留；但当前 xacro 源码中已不再定义该属性。
